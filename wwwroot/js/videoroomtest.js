...
function newRemoteFeed(opaque_id, id, display, room_id) {
	...
	janus.attach(
		{
			...
			onremotestream: function (stream) {
				let video_element = $(`video.device-video[data-room-id=${room_id}]`);
				const serial = video_element.data('serial');
				if (display.includes(serial) && video_element.get(0).srcObject === null) {
					Janus.attachMediaStream(video_element.get(0), stream);
				}
				else {
					video_element.append(`<audio data-display-name="${display}"></audio>`);
					let attach_element = $(`audio[data-display-name=${display}`);
					Janus.attachMediaStream(attach_element.get(0), stream);
				}

			},
			oncleanup: function () {
				element.data('render-state', "stopped");
				$(`audio[data-display-name=${display}`).remove();
...
function attachToRoom(room_id, opaque_id, on_local_stream_cb, on_local_stream_cleanup_cb) {
	return {
		...
		success: function (pluginHandle) {
			...
			var register = {"request": "join", "room": room_id, "ptype": "publisher", "display": this.opaqueId};
				...
				if (event === "joined") {
					...
					if (msg["publishers"] !== undefined && msg["publishers"] !== null) {
						...
						for (var f in list) {
							...
							newRemoteFeed(opaque_id, id, display, room_id);
						}
					}
