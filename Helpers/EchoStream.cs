...
public class EchoStream : Stream
{
    ...
    public override bool CanRead { get { return !m_cancellation_token.IsCancellationRequested; } }
    public override bool CanSeek { get; } = false;
    public override bool CanWrite { get { return !m_cancellation_token.IsCancellationRequested; } }

    public bool CopyBufferOnWrite { get; set; } = false;
    private readonly CancellationToken m_cancellation_token;
    ...
    {
        return Task.Run(() => Write(buffer, offset, count), m_cancellation_token);
    }
    ...
    {
        if (m_Closed || buffer.Length - offset < count || count <= 0 || !CanWrite)
            return;
        ...
        if (!_Buffers.TryAdd(newBuffer, WriteTimeout, m_cancellation_token))
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
        ...
        lock (_lock)
        {
            if (m_count == 0 && _Buffers.Count == 0)
            {
                if (m_Closed)
                    return -1;
                if (_Buffers.TryTake(out m_buffer, ReadTimeout, m_cancellation_token))
                {
                    m_offset = 0;
            ...
            while (count > 0)
            {
                if (m_count == 0)
                {
                    if (_Buffers.TryTake(out m_buffer, 0, m_cancellation_token))
                    {
                        m_offset = 0;
                        m_count = m_buffer.Length;
                    }
