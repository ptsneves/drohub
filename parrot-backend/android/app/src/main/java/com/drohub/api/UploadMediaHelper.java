...
public class UploadMediaHelper {
    public interface Listener {
        void onSuccess(FileEntry f);
        void onUploadError(FileEntry f, String error);
        boolean onProgress(FileEntry f, int percent);
    }
    ...
    public UploadMediaHelper(@NonNull Listener listener,
                             String user_email,
    ...
    public void upload(@NonNull FileEntry file_entry) throws IllegalAccessException {
        ...
        if (_is_preview)
            _file_entry.media_provider.getThumbnail(_file_entry, new UploadPriv(_listener));
        else
            _file_entry.media_provider.getMedia(_file_entry, new UploadPriv(_listener));
    }

    public class UploadPriv implements IPeripheral.IMediaStoreProvider.InputStreamListener {
        private final Listener _listener;
        @Override
        public void onAvailable(FileEntry f, InputStream stream) throws IllegalAccessException {
            uploadPriv(stream);
        }

        @Override
        public void onError(FileEntry f, Exception e) {
            _listener.onUploadError(_file_entry, e.getMessage());
        }
            ...
            try {
                ...
                if (response.has(_RESULT_KEY)) {
                    if (response.optString(_RESULT_KEY).equals(_SEND_NEXT_RESULT_VALUE)) {
                        ...
                        boolean should_continue = _listener.onProgress(_file_entry, _sent_bytes / _file_size);
                        if (should_continue)
                            uploadChunk();
                        else
                            _listener.onUploadError(_file_entry, "Aborted on the request of the user");
                    }
                    else if (response.optString(_RESULT_KEY).equals(_SUCCESS_RESULT_VALUE))
                        _listener.onSuccess(_file_entry);
                }
                else if (response.has(_ERROR_KEY)) {
                    String error = response.optString(_ERROR_KEY);
                    _listener.onUploadError(_file_entry, error);
                }
                else {
                    _listener.onUploadError(_file_entry, "Error but no description");
                }
            }
            catch (JSONException | UnsupportedEncodingException | IllegalAccessException e) {
                _listener.onUploadError(_file_entry, "Unexpected answer uploading file");
            }
        }
        private void uploadChunk() throws IllegalAccessException {
            try {
                ...
                if (_file_stream.read(slice) < 0)
                    _listener.onUploadError(_file_entry, "Chunk read failed.");
                ...
                MultiPartRequest multipart_req = new MultiPartRequest(Request.Method.POST,
                    ...
                    if (error == null) {
                        _listener.onUploadError(_file_entry, "Unspecified error trying to upload");
                        return;
                    }
                    if (error.networkResponse != null) {
                        _listener.onUploadError(_file_entry, String.format("Upload failed with error %d",
                                error.networkResponse.statusCode));
                        return;
                    }
                    if (error.getMessage() != null) {
                        _listener.onUploadError(_file_entry, error.getMessage());
                        return;
                    }
